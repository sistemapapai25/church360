...
class _MuralTab extends ConsumerStatefulWidget {
  final String? filterType;
  const _MuralTab({this.filterType});

  @override
  ConsumerState<_MuralTab> createState() => _MuralTabState();
}

class _MuralTabState extends ConsumerState<_MuralTab> {
  final Set<String> _locallyDeletedPostIds = {};

  void _markPostDeleted(String postId) {
    if (postId.trim().isEmpty) return;
    setState(() => _locallyDeletedPostIds.add(postId));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return postsAsync.when(
      data: (posts) {
        ...
        if (widget.filterType == 'prayer_request') {
          final filteredPosts = visiblePosts
        ...
        final items = <_MuralFeedItem>[
          for (final post in visiblePosts) _MuralFeedItem.post(post),
...
enum _PostOwnerMenuAction { edit, delete }
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              if (isOwner)
                PopupMenuButton<_PostOwnerMenuAction>(
                  tooltip: 'Opções',
                  onSelected: (value) async {
                    if (value == _PostOwnerMenuAction.edit) {
                      await _openEditBottomSheet();
                      return;
                    }
...
enum _ClassifiedOwnerMenuAction { edit, delete }
...
class _ClassifiedCardState extends ConsumerState<_ClassifiedCard>
  ...
  Future<void> _confirmAndSoftDeleteClassified() async {
    ...
    final confirmed =
        await showDialog<bool>(
          ...
          builder: (context) {
            return AlertDialog(
              title: const Text('Remover classificado'),
              content: const Text(
                'Tem certeza que deseja remover este classificado?',
              ),
              actions: [
                TextButton(
                  onPressed: () => Navigator.pop(context, false),
  ...
  Future<void> _openEditClassified() async {
    final colorScheme = Theme.of(context).colorScheme;
    final updated = await showModalBottomSheet<Classified>(
      ...
      builder: (context) => _EditClassifiedSheet(classified: widget.classified),
    );
                    ...
                    PopupMenuButton<_ClassifiedOwnerMenuAction>(
                      ...
                      onSelected: (value) async {
                        if (value == _ClassifiedOwnerMenuAction.edit) {
                          await _openEditClassified();
                          return;
                        }
                        await _confirmAndSoftDeleteClassified();
                      },
                      itemBuilder: (context) => [
                        PopupMenuItem(
...
class _ClassifiedDetailsSheetState
  ...
  Future<void> _confirmAndSoftDelete() async {
    ...
    try {
      await ref
          .read(communityRepositoryProvider)
          .softDeleteClassified(classifiedId: _classified.id);
      ref.invalidate(classifiedsProvider);
      if (!mounted) return;
      Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(
