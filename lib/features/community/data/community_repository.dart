...
class CommunityRepository {
  ...
  bool? _communityPostsUpdatedAtSupported;
  ...
  bool _isMissingUpdatedAtColumn(PostgrestException e) {
    final message = e.message.toLowerCase();
    return e.code == '42703' && message.contains('updated_at') && message.contains('does not exist');
  }
  ...
  Future<void> updatePostContent({
    ...
    Future<void> doUpdate({required bool includeUpdatedAt}) async {
      final payload = <String, dynamic>{
        'content': trimmed,
        if (includeUpdatedAt) 'updated_at': now,
      };
      await _supabase
          .from('community_posts')
          .update(payload)
    ...
    if (_communityPostsUpdatedAtSupported == false) {
      await doUpdate(includeUpdatedAt: false);
      return;
    }

    try {
      await doUpdate(includeUpdatedAt: true);
      _communityPostsUpdatedAtSupported ??= true;
    } on PostgrestException catch (e) {
      if (_isMissingUpdatedAtColumn(e)) {
        _communityPostsUpdatedAtSupported = false;
        await doUpdate(includeUpdatedAt: false);
        return;
      }
      rethrow;
  ...
  Future<void> softDeleteClassified({required String classifiedId}) async {
    final userId = await _effectiveUserId();
    if (userId == null) throw Exception('Usuário não autenticado');
    final now = DateTime.now().toIso8601String();
    await _supabase
