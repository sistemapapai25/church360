...
class _ChurchInfoFormScreenState extends ConsumerState<ChurchInfoFormScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_existingInfo != null) {
        // Atualizar
        await repo.updateChurchInfo(_existingInfo!.id, data);
      } else {
        // Criar
        data['created_at'] = DateTime.now().toIso8601String();
        await repo.createChurchInfo(data);
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_existingInfo != null ? 'Editar Igreja' : 'Cadastrar Igreja'),
        actions: [
          if (_isLoading)
            const Center(
              child: Padding(
