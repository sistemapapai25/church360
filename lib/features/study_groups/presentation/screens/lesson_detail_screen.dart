...
AppStatusTone _lessonStatusTone(LessonStatus status) {
  return switch (status) {
    LessonStatus.published => AppStatusTone.active,
    LessonStatus.draft => AppStatusTone.dropped,
    LessonStatus.archived => AppStatusTone.done,
  };
}

IconData _lessonStatusIcon(LessonStatus status) {
  return switch (status) {
    LessonStatus.published => AppIcons.checkCircle,
    LessonStatus.draft => AppIcons.edit,
    LessonStatus.archived => AppIcons.archive,
  };
}
              ...
              child: Column(
                ...
                children: [
                  ...
                  StatusBadge(
                    ...
                    tone: _lessonStatusTone(lesson.status),
                    icon: _lessonStatusIcon(lesson.status),
                  ),
                  ...
                  if (lesson.content != null) ...[
                    MarkdownBody(
                      ...
                      styleSheet: MarkdownStyleSheet(
                        h1: CommunityDesign.titleStyle(
                          context,
                        ).copyWith(fontSize: 24),
                        h2: CommunityDesign.titleStyle(
                          context,
                        ).copyWith(fontSize: 20),
                        h3: CommunityDesign.titleStyle(
                          context,
                        ).copyWith(fontSize: 18),
                        p: CommunityDesign.contentStyle(context),
                        blockquote: CommunityDesign.contentStyle(context)
                            .copyWith(
                  ...
                  if (lesson.discussionQuestions != null &&
                      lesson.discussionQuestions!.isNotEmpty) ...[
                    const Divider(),
                    const SizedBox(height: 24),
                    Text(
                      'Perguntas para Discussão',
