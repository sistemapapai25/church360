...
class _ScheduleRulesPreferencesScreenState extends ConsumerState<ScheduleRulesPreferencesScreen> {
  ...
  static const _priorityValues = [5, 4, 3, 2, 1];

  String _prioritySymbol(int value) {
    switch (value) {
      case 5:
        return '(++)';
      case 4:
  ...
  Widget _priorityCell(String memberId, String key, int v) {
    return DropdownButton<int>(
      value: v,
      items: _priorityValues
          .map(
            (value) => DropdownMenuItem(
              value: value,
              child: Text('${_prioritySymbol(value)} ${_priorityLabel(value)}'),
            ),
          )
          .toList(),
      selectedItemBuilder: (context) => _priorityValues.map((value) => Text(_prioritySymbol(value))).toList(),
      onChanged: (nv) {
