-- Fix function search paths for batch 10
...
-- Note: detectar_ausencias_consecutivas and processar_alertas_ausencias were NOT found in the codebase and thus cannot be fixed here.
...
BEGIN
  RETURN QUERY
  SELECT
    (SELECT COUNT(*) FROM public.study_lessons WHERE study_group_id = target_group_id AND (SELECT tenant_id FROM public.study_groups WHERE id = target_group_id) = public.current_tenant_id()),
    (SELECT COUNT(*) FROM public.study_lessons WHERE study_group_id = target_group_id AND status = 'published' AND (SELECT tenant_id FROM public.study_groups WHERE id = target_group_id) = public.current_tenant_id()),
    (SELECT COUNT(*) FROM public.study_participants WHERE study_group_id = target_group_id AND (SELECT tenant_id FROM public.study_groups WHERE id = target_group_id) = public.current_tenant_id()),
    (SELECT COUNT(*) FROM public.study_participants WHERE study_group_id = target_group_id AND is_active = true AND (SELECT tenant_id FROM public.study_groups WHERE id = target_group_id) = public.current_tenant_id()),
    CASE
      WHEN (SELECT COUNT(*) FROM public.study_lessons WHERE study_group_id = target_group_id AND status = 'published' AND (SELECT tenant_id FROM public.study_groups WHERE id = target_group_id) = public.current_tenant_id()) = 0 THEN 0
      ELSE (
        ...
        FROM (
          ...
          AND sp.is_active = true
          AND (SELECT tenant_id FROM public.study_groups WHERE id = target_group_id) = public.current_tenant_id()
          GROUP BY sp.user_id
        ) AS rates
      )
    END;
